 ...
 * seed-censo-2023.ts
 *
 * Carga los textos oficiales del Censo de Fluidez y Comprensión Lectora 2023
 * (Plan Lectura y Escritura Mendoza — DGE), uno por año de secundaria,
 * y los asigna en modo PRACTICA a todas las divisiones del año que corresponde.
 *
 * Es idempotente: si el texto ya existe por título, no lo duplica, y las
 * asignaciones se crean solo si no estaban.
 *
 * Uso:  npx tsx src/scripts/seed-censo-2023.ts
 */
...
function divisionesDelAnio(year: number): string[] {
  const letras = year <= 3
    ? ['1ra', '2da', '3ra', '4ta', '5ta']
    : ['1ra', '2da', '3ra', '4ta'];
...
async function main() {
  console.log('Cargando textos del Censo de Fluidez Lectora 2023 (DGE Mendoza)...\n');
